...
<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
  <a href="{{ route('admin.dentists', ['status' => 'all']) }}" class="bg-blue-50 hover:bg-blue-100 transition-colors rounded-lg border border-blue-200 p-4 cursor-pointer">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $totals['total'] }}</p>
      </div>
    </div>
  </a>

  <a href="{{ route('admin.dentists', ['status' => 'active']) }}" class="bg-emerald-50 hover:bg-emerald-100 transition-colors rounded-lg border border-emerald-200 p-4 cursor-pointer">
    <div class="flex items-center gap-3">
  ...
  <a href="{{ route('admin.dentists', ['status' => 'inactive']) }}" class="bg-red-50 hover:bg-red-100 transition-colors rounded-lg border border-red-200 p-4 cursor-pointer">
    <div class="flex items-center gap-3">
      ...
      <div>
        ...
        <p class="text-2xl font-bold text-slate-800">{{ $totals['inactive'] }}</p>
      </div>
    </div>
  </a>
    ...
    <table class="min-w-full text-sm table-hover">
      ...
      <tbody class="divide-y divide-slate-200">
        @forelse($dentists as $d)
          ...
          @endphp
          <tr class="{{ !$isActive ? 'bg-slate-50 opacity-75' : '' }}">
            <td class="px-4 py-3">
              <div class="flex items-center gap-3">
                <div class="avatar">
            ...
            <td class="px-4 py-3">
              <div class="flex items-center justify-end gap-1">
                ...
                <form method="post" action="{{ route('admin.dentists.toggle',$d) }}" class="inline">
                  @csrf
                  @php
                    $isActive = (int)($d->is_active ?? $d->status ?? 0) === 1;
                  @endphp
                  @if($isActive)
                    <button class="btn btn-ghost btn-sm text-red-600 hover:bg-red-50" type="submit" 
                            onclick="return confirm('¿Desactivar odontólogo? Su usuario de sistema también será suspendido.');"
                            title="Desactivar">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>
                    </button>
                  @else
                    <button class="btn btn-ghost btn-sm text-emerald-600 hover:bg-emerald-50" type="submit" 
                            onclick="return confirm('¿Reactivar odontólogo? Su usuario recuperará el acceso.');"
                            title="Activar">
