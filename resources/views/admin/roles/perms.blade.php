      ...
      <div class="border-b border-slate-200 pb-4">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mt-2">
          <p class="text-sm text-slate-600">
            Asigne permisos al rol: <span class="font-medium text-blue-600">{{ $role->name }}</span>
            @if($role->label)
    ...
    <form method="post" action="{{ route('admin.roles.perms.update', $role) }}" class="card">
      ...
      <div class="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            ...
            <div>
              <p class="text-sm font-medium text-blue-800">Permisos seleccionados: <span id="selected-count">0</span></p>
              <p class="text-xs text-blue-600">Marque los permisos que desea asignar a este rol.</p>
            </div>
          </div>
          <button 
            type="button" 
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        @foreach($perms as $permission)
          <label class="flex items-start gap-3 border border-slate-300 rounded-lg p-4 hover:bg-slate-50 transition-colors cursor-pointer">
            <input 
              type="checkbox" 
              name="perms[]" 
              value="{{ $permission->id }}" 
            ...
            >
            <div class="flex-1">
              <div class="font-medium text-slate-800 mb-1">{{ $permission->name }}</div>
              @if($permission->label)
                <div class="text-sm text-slate-600 mb-2">{{ $permission->label }}</div>
              @endif
