  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div class="card mb-6">
      <div class="border-b border-slate-200 pb-4">
        ...
        <p class="text-sm text-slate-600 mt-1">Modifique los detalles de la plantilla de consentimiento.</p>
      </div>
    </div>

    <form method="post" action="{{ route('admin.consents.templates.update', $template) }}" class="card">
      @csrf @method('PUT')
      ...
      <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6">
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span class="text-slate-600">{{ $template->created_at->format('d/m/Y H:i') }}</span>
          </div>
          <div>
            <span class="font-medium text-slate-700">Última actualización:</span>
            <span class="text-slate-600">{{ $template->updated_at->format('d/m/Y H:i') }}</span>
          </div>
        </div>
      </div>
