...
@section('header-actions')
  <a href="{{ route('admin.billing') }}" class="btn btn-ghost">Volver</a>
@endsection

@section('content')
<form method="post" action="{{ route('admin.billing.store') }}" class="space-y-4" id="invoice-form">
  @csrf
  ...
  <div class="flex items-center gap-2">
    ...
    <span class="text-xs text-slate-500">Se emitirá automáticamente.</span>
  </div>
</form>
...
<script>
  ...
  function recalcRow(el) {
    ...
    tr.querySelector('.row-total').textContent = (qty * unit).toFixed(2);
    recalcTotals();
  }
  ...
  function addItemRow() {
    rowIndex++;
    const url = @json(route('admin.billing.create')); // solo para base path
    // Clon simple del primer row desde plantilla embebida:
    fetchTemplate(rowIndex);
  }

  async function fetchTemplate(idx) {
    // Render mínimo desde el cliente (sin blade): creamos la fila vacía
    const services = @json($services->map(fn($s)=>['id'=>$s->id,'name'=>$s->name,'price'=>$s->price]));
    const tr = document.createElement('tr');
    tr.className = 'item-row border-b';
    tr.innerHTML = `
      <td class="px-2 py-2">
        <select name="items[${idx}][service_id]" class="border rounded px-2 py-1" onchange="onServiceChange(this)">
          <option value="">— Servicio —</option>
          ${services.map(s=>`<option value="${s.id}" data-price="${s.price}">${s.name}</option>`).join('')}
        </select>
        <input type="text" name="items[${idx}][description]" placeholder="Descripción" class="mt-1 w-full border rounded px-2 py-1">
      </td>
      <td class="px-2 py-2"><input type="number" name="items[${idx}][quantity]" value="1" min="1" class="w-20 border rounded px-2 py-1" oninput="recalcRow(this)"></td>
      <td class="px-2 py-2"><input type="number" name="items[${idx}][unit_price]" value="0.00" min="0" step="0.01" class="w-28 border rounded px-2 py-1" oninput="recalcRow(this)"></td>
      <td class="px-2 py-2 w-28 text-right"><span class="row-total">0.00</span></td>
      <td class="px-2 py-2 text-right"><button type="button" class="btn btn-danger" onclick="removeItemRow(this)">Eliminar</button></td>
    `;
