...
<body>
    ...
    <table class="ceot-stats">
        <tr>
            <td><span class="ceot-stat-value">Bs {{ number_format($totalInvoiced, 2) }}</span><span
                    class="ceot-stat-label">Total ventas</span></td>
            <td><span class="ceot-stat-value" style="color:#08785c">Bs {{ number_format($totalPaid, 2) }}</span><span
                    class="ceot-stat-label">Total recaudado</span></td>
            <td><span class="ceot-stat-value" style="color:#956000">Bs {{ number_format($totalPending, 2) }}</span><span
                    class="ceot-stat-label">Por cobrar</span></td>
        </tr>
    </table>
    ...
    @if(isset($paymentsByMethod) && $paymentsByMethod->count())
        ...
        @endphp
        <div
            style="margin: 10px 0 16px; padding: 10px 14px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px;">
            <div
                style="font-size: 10px; font-weight: 700; color: #475569; text-transform: uppercase; letter-spacing: .6px; margin-bottom: 8px;">
                Desglose por método de pago
            </div>
                ...
                @foreach($paymentsByMethod as $pm)
                    <tr>
                        <td style="padding: 5px 10px; border-bottom: 1px solid #f1f5f9; width: 40%;">
                            <span
                                style="display: inline-block; width: 4px; height: 12px; background: {{ $methodColors[$pm->method] ?? '#94a3b8' }}; border-radius: 2px; margin-right: 8px; vertical-align: middle;"></span>
                            <span
                                style="font-weight: 600; color: #334155;">{{ $methodLabels[$pm->method] ?? ucfirst($pm->method) }}</span>
                        </td>
                        <td style="padding: 5px 10px; border-bottom: 1px solid #f1f5f9; color: #64748b; width: 20%;">
                            {{ $pm->count }} {{ $pm->count === 1 ? 'cobro' : 'cobros' }}
                        </td>
                        <td
                            style="padding: 5px 10px; border-bottom: 1px solid #f1f5f9; text-align: right; font-weight: 700; color: {{ $methodColors[$pm->method] ?? '#334155' }};">
        ...
        <tbody>
            @forelse($invoices as $invoice)
                <tr>
                    ...
                    <td>
                        <span class="font-bold">{{ $invoice->patient->first_name }}
                            {{ $invoice->patient->last_name }}</span>
                        @if($invoice->items->count())
                            <div class="invoice-items">
                                @foreach($invoice->items as $item)
                                    <div>{{ $item->quantity }}x {{ $item->description ?: ($item->service->name ?? 'Servicio') }} -
                                        Bs {{ number_format($item->total, 2) }}</div>
                                @endforeach
                            </div>
                        @endif
                    </td>
                    ...
                    <td class="text-right" style="color:{{ $invoice->calc_balance > 0.01 ? '#956000' : '#536578' }}">
                        {{ number_format($invoice->calc_balance, 2) }}</td>
                    <td><span
                            class="ceot-badge {{ $statusClasses[$invoice->status] ?? 'badge-slate' }}">{{ $statusLabels[$invoice->status] ?? $invoice->status }}</span>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="7" class="text-center muted">No se encontraron comprobantes con los filtros seleccionados.
                    </td>
                </tr>
            @endforelse
        </tbody>
