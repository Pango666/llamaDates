...
@section('header-actions')
  <a href="{{ route('admin.patients.show',$patient) }}" class="btn btn-ghost flex items-center gap-2">
  ...
  <div class="max-w-4xl mx-auto">
    ...
    <div class="card mb-6">
      ...
      <div class="flex flex-wrap gap-2 mt-3">
        <button type="button" data-filter="all" class="filter-btn active btn btn-ghost text-xs bg-blue-100 text-blue-700 border-blue-200">
          Todos
        </button>
        <button type="button" data-filter="appointment" class="filter-btn btn btn-ghost text-xs">
          Citas
        </button>
        <button type="button" data-filter="note" class="filter-btn btn btn-ghost text-xs">
          Notas
        </button>
        <button type="button" data-filter="diagnosis" class="filter-btn btn btn-ghost text-xs">
          Diagnósticos
        </button>
        <button type="button" data-filter="treatment" class="filter-btn btn btn-ghost text-xs">
          Tratamientos
        </button>
        <button type="button" data-filter="odontogram" class="filter-btn btn btn-ghost text-xs">
          Odontogramas
        </button>
        <button type="button" data-filter="consent" class="filter-btn btn btn-ghost text-xs">
          Consentimientos
        </button>
        <button type="button" data-filter="attachment" class="filter-btn btn btn-ghost text-xs">
          Archivos
        </button>
      </div>
    </div>
              ...
              <li class="relative pl-16 timeline-item" data-type="{{ $e['type'] ?? 'other' }}">
                ...
                <div class="bg-white rounded-lg border border-slate-200 p-4 hover:shadow-sm transition-shadow">
                  <div class="flex items-start justify-between gap-4 mb-2">
                    ...
                    <div class="shrink-0">
                      <a href="{{ $url }}" 
                         class="btn btn-ghost flex items-center gap-1 hover:bg-{{ $typeConfig['color'] }}-50 hover:text-{{ $typeConfig['color'] }}-700 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  filterButtons.forEach(button => {
    button.addEventListener('click', function() {
      ...
      const counter = document.querySelector('.text-slate-500');
      if (counter) {
        counter.textContent = `${visibleItems.length} evento(s) registrado(s)`;
      }
    });
  });
});
