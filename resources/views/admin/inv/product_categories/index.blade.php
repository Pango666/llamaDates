...
@section('header-actions')
  <a href="{{ route('admin.inv.product_categories.create') }}" class="btn bg-blue-600 text-white hover:bg-blue-700 flex items-center gap-2">
  ...
  <div class="max-w-5xl mx-auto">
    <div class="card mb-6">
      <div class="border-b border-slate-200 pb-4">
        ...
        <p class="text-sm text-slate-600 mt-1">Clasifica medicamentos, materiales e insumos (Analgesia, Antibióticos, Material restaurador, etc.).</p>
      </div>
    </div>
    ...
    <div class="card mb-6">
      <form method="get" class="grid gap-4 md:grid-cols-4 md:items-end">
        ...
        <div class="flex gap-2">
          ...
          @if($q !== '')
            <a href="{{ route('admin.inv.product_categories.index') }}" class="btn btn-ghost flex items-center gap-2">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="card p-0 overflow-hidden">
            ...
            <tbody class="divide-y divide-slate-200">
              @foreach($categories as $category)
                <tr class="hover:bg-slate-50">
                  ...
                  <td class="px-4 py-3">
                    @if($category->code)
                      <code class="px-2 py-1 bg-slate-100 rounded text-xs text-slate-700">{{ $category->code }}</code>
                    @else
                  ...
                  <td class="px-4 py-3">
                    <div class="flex items-center justify-end gap-2">
                      <a href="{{ route('admin.inv.product_categories.edit', $category) }}" class="btn btn-ghost text-sm flex items-center gap-1">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                        Editar
                      </a>
                      <form method="post" action="{{ route('admin.inv.product_categories.destroy', $category) }}" onsubmit="return confirm('¿Eliminar categoría?');">
                        @csrf @method('DELETE')
      ...
      @else
        <div class="text-center py-10">
          <p class="text-slate-600">No hay categorías registradas.</p>
        </div>
      @endif
    </div>
