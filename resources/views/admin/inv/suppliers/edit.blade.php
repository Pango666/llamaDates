  ...
  <div class="max-w-2xl mx-auto">
    ...
    <div class="card mb-6">
      <div class="border-b border-slate-200 pb-4">
        ...
        <p class="text-sm text-slate-600 mt-1">Modifique la información del proveedor.</p>
      </div>
    </div>

    <form method="post" action="{{ route('admin.inv.suppliers.update', $supplier) }}" class="card">
      @csrf @method('PUT')
      @include('admin.inv.suppliers.form-fields')
      
      {{-- Información del proveedor --}}
      <div class="bg-slate-50 border border-slate-200 rounded-lg p-4 mb-6 md:col-span-2">
        <div class="grid grid-cols-2 gap-4 text-sm">
          <div>
            <span class="font-medium text-slate-700">Creado:</span>
            <span class="text-slate-600">{{ $supplier->created_at->format('d/m/Y H:i') }}</span>
          </div>
          <div>
            <span class="font-medium text-slate-700">Última actualización:</span>
            <span class="text-slate-600">{{ $supplier->updated_at->format('d/m/Y H:i') }}</span>
          </div>
        </div>
      </div>
